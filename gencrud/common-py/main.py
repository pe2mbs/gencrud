...
def discoverModules():
    ...
    for package in os.listdir( currentFolder ):
        ...
        if os.path.isdir( packageFolder ) and os.path.isfile( os.path.join( packageFolder, '__init__.py' ) ):
            ...
            logger.debug( "Loading module: {}".format( module.__name__ ) )
...
def logMappedDrives():
    ...
    for drive in drives.split('\000')[:-1]:
        try:
            ...
            logger.debug( "drive: {} - {} :: {}".format( drive, result[4], result[0] ) )

        except pywintypes.error as exc:
            logger.debug( "drive: {} - {}".format( drive, exc.strerror ) )
...
def registerApi( app, cors ):
    ...
    logger.debug( 'Register Menu route' )
...
def registerExtensions():
    API.app.logger.debug( 'Register extensions' )
    for module in listModules:
...
def registerShellContext():
    API.app.logger.debug( 'Register shell context' )
    for module in listModules:
...
def registerCommands():
    API.app.logger.debug( 'Register extra commands' )
    for module in listModules:
