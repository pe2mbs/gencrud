...
class TemplateAction( object ):
    ...
    def buttonObject( self ):
        ...
        if function != '':
            BUTTON_STR = '''<button {button} color="primary" ({on})="{function}" id="{objname}.{name}">{content}</button>'''

        else:
            # BUTTON_STR = '''<a {button} color="primary" routerLink="/{route}" {params} id="{objname}.{name}">{content}</a>'''
            BUTTON_STR = '''<a {button} color="primary" ({on})="router.navigate( [ '/{route}', {params} ] )" id="{objname}.{name}">{content}</a>'''
