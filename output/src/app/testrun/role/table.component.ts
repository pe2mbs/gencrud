...
import { AddEditRoleDialog } from './addedit.dialog';
...
export class RoleTableComponent implements OnInit
{
    ...
    dataSource:       RoleDataSource  | null;
    index:            number;
    id:               number;
    ...
    addNew() 
    {
        let record: RoleRecord = new RoleRecord;
        const dialogRef = this.dialog.open( AddEditRoleDialog,
        {
            data: { record: record,
                    mode: 'add' },
        } );
    ...
    startEdit( i: number, record: RoleRecord )
    {
        this.dataService.lockRecord( record );
        this.id = record.D_ROLE_ID;
        ...
        dialogRef.afterClosed().subscribe( result => 
        {
            ...
            if ( result === 1 ) 
            {
                ...
                const foundIndex = this.dataService.dataChange.value.findIndex( x => x.D_ROLE_ID === this.id );
    ...
    deleteItem( i: number, record: RoleRecord )
    {
        this.dataService.lockRecord( record );
        this.id = record.D_ROLE_ID;
